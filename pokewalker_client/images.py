...
if _has_pil:
    ...
    def generate_name_image(name: str, width: int = 80) -> bytes:
        """
        Render a text string as a walker-format name image.

        Args:
            name:  Text to render (e.g. "Pikachu")
            width: Canvas width in pixels — 80 for Pokémon names, 96 for item names

        Returns:
            Encoded image bytes (width * 16 / 4 bytes)
        """
        from PIL import Image as PILImage, ImageDraw, ImageFont

        img = PILImage.new("L", (width, 16), color=255)
        ...
        draw.text((x + 1, y), label, fill=0, font=font)

        img = img.point(lambda v: 0 if v < 128 else 255)

        return image_to_walker_format(img, width, 16, dither=False)
