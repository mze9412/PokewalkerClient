...
class TestChecksum:
    ...
    def test_checksum_basic(self):
        """Test basic checksum calculation against known packet value."""
        data = bytes([
            0x20,  # command (index 0, even)
            0x01,  # extra (index 1, odd)
            0x00,  # checksum hi (index 2) - skipped
            0x00,  # checksum lo (index 3) - skipped
    ...
    def test_checksum_real_packet(self):
        """Test checksum against a real captured 0xFA packet from reverse-pokewalker docs."""
        # cmd=0xFA, arg=0x01, checksum=0x9008, session=0x3F57CF34
        data = bytes([0xFA, 0x01, 0x00, 0x00, 0x3F, 0x57, 0xCF, 0x34])
        assert calculate_checksum(data) == 0x9008

    def test_checksum_result_in_range(self):
        """Checksum must always be a 16-bit value."""
        data = bytes([0xFF] * 8)
        checksum = calculate_checksum(data)
